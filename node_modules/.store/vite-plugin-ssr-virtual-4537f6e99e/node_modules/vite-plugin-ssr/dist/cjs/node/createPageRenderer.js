...
function createPageRenderer(_deprecated) {
    (0, utils_1.assertWarning)(false, "`createPageRenderer()` is not needed anymore (vite-plugin-ssr now automatically determines the options). To avoid this warning, replace `const renderPage = createPageRenderer(/*...*/)` with the drop-in replacement `import { renderPage } from 'vite-plugin-ssr'`.", { onlyOnce: true });
