...
exports.extractStylesAddQuery = exports.extractStylesRE = exports.extractStylesPlugin = void 0;
const utils_1 = require("../utils");
const parseEsModules_1 = require("../parseEsModules");
const extractStylesAddQuery_1 = require("./extractStylesPlugin/extractStylesAddQuery");
Object.defineProperty(exports, "extractStylesAddQuery", { enumerable: true, get: function () { return extractStylesAddQuery_1.extractStylesAddQuery; } });
const extractStylesRE = /(\?|&)extractStyles(?:&|$)/;
exports.extractStylesRE = extractStylesRE;
...
const DEBUG = (_a = process.env.DEBUG) === null || _a === void 0 ? void 0 : _a.includes('extractStyles');
function extractStylesPlugin() {
    let root;
    return [
        // Remove all JS from `.page.server.js` files and `?extractStyles` imports, so that only CSS remains
        {
            ...
            async transform(src, id, options) {
                const isServerSide = (0, utils_1.isSSR_options)(options);
                if (!isTransformTarget(id, isServerSide)) {
                    return;
                }
                (0, utils_1.assert)(!isServerSide);
...
exports.extractStylesPlugin = extractStylesPlugin;
function isTransformTarget(id, isServerSide) {
...
function emptyModule(id, importer) {
    debug('NUKED', id, importer);
    return EMPTY_MODULE_ID;
}
function transformedId(id, importer) {
    const fileExtension = (0, utils_1.getFileExtension)(id);
    if (!fileExtension) {
        return emptyModule(id, importer);
    }
    debug('TRANSFORMED', id, importer);
    return (0, extractStylesAddQuery_1.extractStylesAddQuery)(id);
}
function getImports(esModules) {
    ...
    a === -1 &&
        ...
        (0, utils_1.assert)(importee);
        imports.push(`import '${importee}';`);
    });
    return imports;
}
function debug(operation, id, importer) {
    if (!DEBUG) {
        return;
    }
    console.log('');
