...
import { assert, hasProp, isBrowser, isCallable, isObject, assertPosixPath, cast, assertWarning, assertUsage, unique, slice, } from './utils';
...
function format(pageFilesExports) {
    ...
    assert(hasProp(pageFilesExports.pageFilesLazy, '.page.route') || hasProp(pageFilesExports.pageFilesEager, '.page.route'));
...
function createObjectWithDeprecationWarning() {
    return new Proxy({}, {
        get(...args) {
            assertWarning(false, '`pageContext.pageExports` is deprecated. Use `pageContext.exports` instead, see https://vite-plugin-ssr.com/exports', { onlyOnce: true });
...
function assertExports(pageFiles, customExports) {
    customExports.forEach((customExportName) => {
        assertUsage(!Object.keys(VPS_EXPORTS).includes(customExportName), `\`export { customExports }\` contains \`${customExportName}\` which is forbidden because it is an '\export\` already used by vite-plugin-ssr.`);
    });
    pageFiles.forEach((p) => {
        var _a;
        Object.keys((_a = p.fileExports) !== null && _a !== void 0 ? _a : {}).forEach((exportName) => {
            ...
            assertWarning(customExports.includes(exportName), `Unknown \`export { ${exportName} }\` at ${p.filePath}. See https://vite-plugin-ssr/customExports if you want to define a custom export.`, { onlyOnce: `unkown-export-${exportName}` });
        });
    });
}
function getStringUnion(exportsAll, propName) {
    ...
    return ((_b = (_a = exportsAll[propName]) === null || _a === void 0 ? void 0 : _a.map((e) => {
        assertUsage(hasProp(e, 'exportValue', 'string[]'), `\`export { ${propName} }\` of ${e.filePath} should be an array of strings.`);
