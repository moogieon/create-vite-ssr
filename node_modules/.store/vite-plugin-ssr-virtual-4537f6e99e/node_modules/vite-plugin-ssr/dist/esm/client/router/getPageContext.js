...
import { assert, assertUsage, assertWarning, getFileUrl, hasProp, isPlainObject, objectAssign, getProjectError, } from './utils';
...
async function getPageContextPageNavigation(pageContext) {
...
async function hasOnBeforeRenderServerSide(pageContext) {
    const pageFilesServerExportNames = pageContext._pageFilesAll.filter((p) => p.fileType === '.page.server' && p.isRelevant(pageContext._pageId));
    await Promise.all(pageFilesServerExportNames.map((p) => { var _a; return (_a = p.loadExportNames) === null || _a === void 0 ? void 0 : _a.call(p); }));
    ...
    alreadySet.forEach((prop) => {
        if (prop in pageContext) {
            ...
            if (BUILT_IN_CLIENT_ROUTER.includes(prop)) {
                ...
                assertWarning(false, `\`pageContext.${prop}\` is already available in the browser when using \`useClientRouter()\`; including \`${prop}\` in \`passToClient\` has no effect.`, { onlyOnce: true });
            }
            else {
