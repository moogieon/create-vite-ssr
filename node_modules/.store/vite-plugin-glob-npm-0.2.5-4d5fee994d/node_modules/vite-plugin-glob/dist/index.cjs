...
async function toAbsoluteGlob(glob, root, dirname, resolveId) {
  ...
  if (glob.startsWith("./"))
    return pre + path.posix.join(dirname, glob.slice(2));
  if (glob.startsWith("../"))
    return pre + path.posix.join(dirname, glob);
  if (glob.startsWith("**"))
    return pre + glob;
...
async function transform(code, id, root, resolveId, options) {
  ...
  const staticImports = (await Promise.all(matches.map(async ({ globsResolved, isRelative, options: options2, index, start, end }) => {
    ...
    let query = !options2.query ? "" : typeof options2.query === "string" ? options2.query : ufo.stringifyQuery(options2.query);
